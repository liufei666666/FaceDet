...
class FaceBoxes(nn.Module):
    ...
    def __init__(self, phase, size, num_classes):
        ...
        self.conv2_3 = conv_dw(128, 256, 2)
    ...
    def multibox(self, num_classes):
        ...
        # loc_layers += [nn.Conv2d(512, 1 * 4, kernel_size=3, padding=1)]
        # conf_layers += [nn.Conv2d(512, 1 * num_classes, kernel_size=3, padding=1)] 
        # loc_layers += [nn.Conv2d(128, 2 * 4, kernel_size=3, padding=1)]
        # conf_layers += [nn.Conv2d(128, 2 * num_classes, kernel_size=3, padding=1)]
    ...
    def forward(self, x):
        ...
        #_a['conv1'].tic()
        x = self.conv1(x)
        x = F.max_pool2d(x, kernel_size=2, stride=2, padding=0, ceil_mode=True)
        #_a['conv1'].toc()
        ...
        # loc_reshape = loc.view(loc.size(0), -1, 4)
        # cof_reshape = conf.view(conf.size(0), -1, self.num_classes)
        #_a['pre'].toc()
        if self.phase == "test":
        ...
        #return loc_reshape,cof_reshape
        return output
#writer.close()
